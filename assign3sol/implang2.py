...
grammar = """
  ?start: stmt
   ...
   stmt: "var" ID "=" aexpr         -> decl
       | ID "=" aexpr               -> assign
       | "if" "(" aexpr ")" stmt ["else" stmt] -> ifstmt
       | "while" "(" aexpr ")" stmt -> whstmt
       | "print" "(" aexpr ")"      -> prstmt
       | "{" stmt (";" stmt)* "}"  -> block      
...
class Eval(Interpreter):
    ...
    def left_less_than_right(self, x, y): #return true if the left value is less than the right value
        return self.visit(x) < self.visit(y)
    
    def equals(self, x, y): #return true if the left value is equal to the right value
        return self.visit(x) == self.visit(y)
    
    def range(self, x, y): #create a range of ints from x to y (exclusive)
        return range(self.visit(x), self.visit(y))
    
    def for_loop(self, var, rng, stmt): #for loop that iterates through a range of ints
        self.env = self.env.open_scope()
        try:
            for i in self.visit(rng):
                self.env.extend(var, i)
                self.visit(stmt)
        finally:
